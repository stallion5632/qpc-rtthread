...
#ifdef Q_RT_DEBUG
        rt_kprintf("[QF_MEMPOOL] Freeing event: pool=%u, ptr=%p\n", poolId, e);
#endif /* Q_RT_DEBUG */

        /* 释放到正确的内存池 */
        rt_mp_free(&l_memPools[poolId].pool, (void *)e);
        ++l_poolStats.deallocations;

        /* 调试填充在释放之后进行，避免访问已释放的内存 */
#ifdef Q_RT_DEBUG
        rt_kprintf("[QF_MEMPOOL] Event freed successfully\n");
#endif /* Q_RT_DEBUG */
    }
}
